...
### 1.2 缓存行
由于共享变量在[CPU三级缓存](/markdown/thread/volatile.md)中的存储是以缓存行为单位，一个缓存行可以存储多个变量（存满当前缓存行的字节数）；而CPU对缓存的修改又是以缓存行为最小单位的，那么就会出现上诉的伪共享问题。
...
### 3.2 Disruptor通过以下设计来解决队列速度慢的问题：
|:-|
|(1)环形数组结构|
